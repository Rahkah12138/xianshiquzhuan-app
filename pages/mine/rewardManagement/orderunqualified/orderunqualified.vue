<template>
	<view>
				...
				<view class="steplist" v-for="(e,f) in detail.tasksteps" :key="f">
					...
					<view v-if="e.ids==1||e.ids==3||e.ids==4" class="tr0">
						...
						<block v-for="(g,h) in e.url" :key="h">
							<image :class="detail.type==5? `imgs${h}`:'imgs'+(e.ids==1?'1':'2')"  :src="global.url+g" mode="aspectFill"
								@click="perviewImg(e.url)">
							<text v-if="h==1&&detail.type==5"  class="imgs1Word"  @click.stop="chooseImg(f)" style="display: block;width: 200rpx;height: 60rpx;background-color: #017ff0;line-height: 60rpx;text-align: center;">重新扫码上传</text>
								</image>
		...
		<!-- 按钮 -->
		<view v-if="detail.type==5" class="btns">
			<view @click="clickLeft">
				<image :src="'/static/'+(detail.type==5?'search/del':'detail/huan')+'.png'" mode="widthFix"></image>
				{{detail.type==5?'放弃任务':''}}
			</view>
			<view @click="clickRight" v-if="detail.type==5" style="color: #FFFFFF;">重新提交任务</view>
		</view>
		...
		<view v-show="isJoin" @touchmove.stop.prevent="stopSlide">
			...
			<view class="tip-content">
				<view class="tip-back">
					<text style="color: #FFFFFF;">赚赏金 就三步</text>
				...
				uni.request({
					...
					success: (res) => {
						if (res.data.code == 1) {
							...
							if(!this.detail.image.includes('http')){
