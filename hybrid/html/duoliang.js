(function(w) {
	...
	var dysdk = {
		...
		downloadAndInstall: function(down_url, packName) {
			...
			var index = down_url.lastIndexOf('/') > down_url.lastIndexOf('?') ? down_url.lastIndexOf('/') :
				down_url.lastIndexOf(
					'?');
			...
			dtask.addEventListener("statechanged", function(task, status) {
				...
				switch (task.state) {
					...
					case 2: // 已连接到服务器
						...
						break;
					case 3: // 已接收到数据
						var cur_precent = parseInt(100 * task.downloadedSize / task
							.totalSize) //Math.round(task.downloadedSize * 100 / task.totalSize)
